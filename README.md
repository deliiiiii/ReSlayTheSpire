...
| 子类名                   | Inspector                                                    | 执行                                           | 返回             |
| ------------------------ | ------------------------------------------------------------ | ---------------------------------------------- | ---------------- |
| **GuardNodeCompare**     | 拖入数据黑板，指定黑板类中指定值（反射自动拿到名字与类型），指定比较方法（==,!=,>,<,>=,<=），指定目标值（反射自动拿到类型） | 按指定方法比较黑板上的指定值FromValue和ToValue | 反转子节点返回值 |
...
PPS：节点的端口连接限制（输入or输出、连单个or多个、连接目标类型、端口名）保存在文件Assets/DataTree/PortToDrawConfig.asset，非开发者不要修改。
...
<u>为避免k2这个string被删掉而懒得查配置文件，Inspector提供以下两个bool（其实相当于按钮）</u>
